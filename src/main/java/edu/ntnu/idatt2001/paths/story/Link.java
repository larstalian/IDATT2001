 ...
 * describes
 * the link, and a reference to the target passage. Links may also include a list of optional
 * Actions that are performed when the link is followed. Actions are represented as instances of
 * actions implementing the {@link Action} interface. </p>
 *
...
public class Link {
  ...
  public Link(String text, String ref) {
    if (text.length() < 2 || text.length() > TEXT_MAX_LENGTH) {
      throw new IllegalArgumentException(
          "Text and must be between " + TEXT_MIN_LENGTH + " and " + TEXT_MAX_LENGTH
              + " characters.");
    }
    if (ref.length() < REF_MIN_LENGTH || ref.length() > REF_MAX_LENGTH) {
      throw new IllegalArgumentException(
   ...
   * `Link` object.
   * They are used to ensure that the `text` and `ref` fields are within the expected range of
   * lengths. </p>
   *
