...
class PlayerTest {
    ...
    void testAddToInventory_ShouldThrowExceptionWhenInventoryIsFull() {
        IntStream.range(0, MAX_INVENTORY_SIZE - player.getInventory().size()).forEach(i -> player.addToInventory("Item"));
    ...
    void testNameLengthShouldNotBeLongerThanDefined() {
        ...
        assertThrows(IllegalArgumentException.class, () -> new Player(tooLongString, 100, 0, 0));
    }

    @Test
    void testNameLengthShouldNotBeShorterThanDefined() {
        String tooShortString = new String(new char[MIN_NAME_LENGTH - 1]).replace("\0", "a");
        assertThrows(IllegalArgumentException.class, () -> new Player(tooShortString, 100, 0, 0));
    }
}